...
public class ApplicationConfiguration {
    ...
    @Bean("webSearchRagChatClientConfig")
    public ChatClient webSearchRagChatClientConfig(ChatClient.Builder chatClientBuilder, ChatMemory chatMemory, TavilyConfig tavilyConfig, RestClient restClient) {
        SimpleLoggerAdvisor simpleLoggerAdvisor = new SimpleLoggerAdvisor();
        TokenUsageLoggerAdvisor tokenUsageLoggerAdvisor = new TokenUsageLoggerAdvisor();
        MessageChatMemoryAdvisor memoryAdvisor = MessageChatMemoryAdvisor.builder(chatMemory).build();
        RetrievalAugmentationAdvisor webSearchRagAdvisor = RetrievalAugmentationAdvisor.builder().documentRetriever(
                WebSearchDocumentRetriever.builder().withRestClient(restClient).withTavilyConfig(tavilyConfig).withResultLimit(5).build()
        ).build();
        return chatClientBuilder.defaultAdvisors(simpleLoggerAdvisor, tokenUsageLoggerAdvisor, memoryAdvisor, webSearchRagAdvisor).build();
    }
